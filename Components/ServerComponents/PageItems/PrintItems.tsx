...
import { ItemSelect, ItemMenu } from "@/Components/ClientComponents/ItemSelects/ItemSelect";
import { supabase } from "@/lib/supabase";

export default async function PrintItems() {
  const { data: files, error } = await supabase.storage.from("print-assets").list("icons");
  if (error) {
  ...
  const iconData = files?.filter((file) => file.name.endsWith(".png"))
                          .map((file) => {
                          const publicUrlData = supabase.storage.from("print-assets/icons").getPublicUrl(file.name);
                          return {title: file.name.slice(0, -4), src: publicUrlData.data.publicUrl,
                            href: `/Prints/PrintPreview?print=${file.name.slice(0, -4)}`
                          }});
